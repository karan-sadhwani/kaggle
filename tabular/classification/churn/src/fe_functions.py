...
def target_encode(train_df, test_df, cols, TARGET, i, method='mean'):
    
    temp_df = train_df.copy()
    temp_df.loc[:,'Semana'] = temp_df['Semana'] + i
    temp_df = temp_df.groupby(cols)[TARGET].agg([method]).reset_index().rename(
                                                    columns={method: ('_'.join(cols))+'_target_'+method+'_lag_'+str(i)})
    print(temp_df)
    train_df = pd.merge(train_df, temp_df,how = 'left', on = cols,
                        left_index = False, right_index=False)
    test_df = pd.merge(test_df, temp_df, how = 'left', on = cols,
                        left_index = False, right_index=False)
...
#         test_df[col] = test_df[col].astype('category')
    ...
    #     # calculate mean
    #     mean_df = pd.DataFrame({k:df_train.groupby(v)['log_demand'].mean()}).reset_index()
    #     # unlog mean
    #     mean_df[k] = np.expm1(mean_df[k])
    #     # merge to validation set
    #     df_valid = df_valid.merge(mean_df, how='left', on=v)
    #     # update prediction column
    #     df_valid.loc[df_valid[k].notnull(), 'prediction'] = df_valid[k]
    #     df_valid['diff'] = df_valid['Demanda_uni_equil'] - df_valid['prediction']
    #     # get logged target and predictions
    #     df_valid['log_target'] = np.log1p(df_valid['Demanda_uni_equil'])
    #     df_valid['log_prediction'] = np.log1p(df_valid['prediction'])
    #     df_valid['log_diff'] = df_valid['log_target'] - df_valid['log_prediction']
    #     # calculate rmse
    #     rmse = (v, ((df_valid['log_target'] - df_valid['log_prediction']) ** 2).mean() ** .5)
    #     print(rmse)
...
def log_one_plus(df, cols):
    """ Takes log of a variable
    Criteria - skewed distributions and 0 values
    Model Crtieria
    """
    for col in cols:
